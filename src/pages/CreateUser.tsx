...
const CreateUser = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm, setErrors }) => {
      try {
        const { name, surname, email, gender, status } = values;
        ...
        if (response.status === 201) {
          ...
          resetForm();
          setSubmitting(false);
          enqueueSnackbar("User created successfully with id: " + id, {
            variant: "success",
          });
          navigate("/users", { replace: true });
        } else {
          setSubmitting(false);
          enqueueSnackbar("Couldn't create a user!", {
            variant: "error",
          });
        }
      } catch (error: any) {
                ...
                <Grid item xs={12}>
                  ...
                  >
                    ...
                    <Button
                      ...
                      color="primary"
                    >
                      {isSubmitting ? <CircularProgress size={26} /> : "CREATE"}
                    </Button>
                  </Box>
                </Grid>
              </Grid>
